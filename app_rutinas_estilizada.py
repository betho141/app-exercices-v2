...
# --- ESTILOS PERSONALIZADOS: FONDO NEGRO ---
st.markdown(
    """
    <style>
    body {
        background-color: rgb(0, 0, 0);  /* Fondo negro puro */
    }
    .stApp {
...
if modo == "Usuario":
    ...
    if password == "dioses123":
        ...
        if st.button("Guardar nueva rutina"):
            try:
                cur.execute("SELECT id_rutina FROM rutinas WHERE nombre_clave = %s AND nombre_rutina = %s", (usuario_seleccionado, nombre_rutina))
                resultado = cur.fetchone()
                if resultado:
                    id_rutina = resultado[0]
                else:
                    cur.execute("INSERT INTO rutinas (nombre_clave, nombre_rutina, fecha_creacion) VALUES (%s, %s, %s) RETURNING id_rutina", (usuario_seleccionado, nombre_rutina, datetime.now()))
                ...
                for nombre_ejercicio in ejercicios_seleccionados:
                    ...
                    cur.execute("INSERT INTO detalle_rutina (id_rutina, id_ejercicio, repeticiones, series) VALUES (%s, %s, %s, %s)", (id_rutina, id_ej, repeticiones, series))
                conn.commit()
