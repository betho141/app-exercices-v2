                ...
                try:
                    ...
                    for _, row in tabla_editada.iterrows():
                        ...
                        if id_ej in ids_existentes:
                            continue
                        cur = conn.cursor()
                        cur.execute("""INSERT INTO detalle_rutina (id_rutina, id_ejercicio, repeticiones, series)
                                       VALUES (%s, %s, %s, %s)""", (id_rutina_mod, id_ej, int(row["N° Repeticiones"]), int(row["N° Series"])))
                        cur.close()
                        count_agregados += 1
                    conn.commit()
