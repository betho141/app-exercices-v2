...
if modo == "Usuario":
    ...
    if password == "dioses123":
        ...
        if not df_rutinas_usuario.empty:
            ...
            ejercicios_agregar = st.multiselect("Ejercicios a agregar", df_filtrado_mod["nombre"].tolist(), key="multi_agregar")
            reps_mod = st.number_input("Repeticiones", min_value=1, max_value=100, value=10, key="reps_mod")
            series_mod = st.number_input("Series", min_value=1, max_value=20, value=3, key="series_mod")

            if st.button("Agregar ejercicios a rutina"):
                try:
                    ids_existentes = set(df_detalle_actual["id_ejercicio"])
                    count_agregados = 0
                    for nombre_ej in ejercicios_agregar:
                        id_ej = int(df_ejercicios[df_ejercicios["nombre"] == nombre_ej]["id"].values[0])
                        ...
                        cur.execute("""
                            ...
                            VALUES (%s, %s, %s, %s)
                        """, (id_rutina_mod, id_ej, reps_mod, series_mod))
                        cur.close()
                        count_agregados += 1
                    conn.commit()
